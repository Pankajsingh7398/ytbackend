...
const logoutUser = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .clearcookie("asccessToken", accessToken, options)
        .clearcookie("refreshToken", refreshToken, options)
        .json(
            new ApiResponse(200, {
                user: loggedout, accessToken, refreshToken
            },
...
const refressAccessToken = asyncHandler(async (req, res) => {
    ...
    const user = await user.findby_id(decodedToken?._id)
    if (!User) {
    ...
    const { accessTokenccessToken, newRefreshToken } = await generateAccessAndRefreshToken(user._id)
        .status(200)
    ...
    cookie("refreshToken", newRefreshToken, options)
        ...
        throw new ApiError(402,error?.message ||"Refresh invalid Token ");
...
const updateaccountdetails = asyncHandler(async (req, res) => {
    ...
    if(!fullName|| email)
        throw ApiError(401,"full name and email is requird")
    ...
    return res
    .status(200)
    .json(new ApiResponse(200,"account details update seccesfully" ,{user})) 
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    const avatarLocalPath= req.files?.avatar?.[0]?.path;
     ...
     const avatar =await uploadOnCloudinary(avatarLocalPath)
       if(!avatar.url){
        throw ApiError(401,"error while uploading on cloudinary")
       }
...
const updateUsercoverimage = asyncHandler(async (req, res) => {
    ...
    return res
    .status(200)
    .json(new ApiResponse(200,"coverImage updated seccesfully" ,{user})) 
    });
